...
for (int i = 0; i < str.Length; i++)
{
    ...
    else if (current_element.ToString() != ")")
    {
        ...
        else
        {
            ...
            // Лол, это действительно так!
            if (stack.Count > 0 && (stack.Peek().ToString() == "+" || stack.Peek().ToString() == "-" || stack.Peek().ToString() == "*" || stack.Peek().ToString() == "/"))
            {
                while ((stack.Count > 0) && (priority_operation[stack.Peek()] > priority_operation[current_element.ToString()])) // && stack.Count > 0
                {
                    result += stack.Pop();
                }
            }
